...
let mockNetworkChangeCallback: ((state: { isConnected: boolean | null; isInternetReachable: boolean | null }) => void) | null = null;

jest.mock('@react-native-community/netinfo', () => ({
  addEventListener: jest.fn((callback: (state: { isConnected: boolean | null; isInternetReachable: boolean | null }) => void) => {
...
jest.mock('@/shared/ui/Toast', () => {
  ...
  return {
    Toast: ({ visible, title, message, type, onHide }: { visible: boolean; title: string; message: string; type: string; onHide: () => void }) => {
...
jest.mock('@/shared/ui/AlertModal', () => {
  const { Text } = jest.requireActual<typeof import('react-native')>('react-native');
  return {
    AlertModal: ({ visible, title, message, type, onConfirm }: { visible: boolean; title: string; message: string; type: string; onConfirm: () => void }) => {
      if (!visible) return null;
...
jest.mock('@/shared/ui/NetworkBanner', () => {
  const { Text } = jest.requireActual<typeof import('react-native')>('react-native');
  return {
    NetworkBanner: ({ isOffline }: { isOffline: boolean }) => {
...
describe('NotificationProvider', () => {
  ...
  it('deve exibir e ocultar o Toast corretamente chamando o context', async () => {
    ...
    await fireEvent.press(getByTestId('btn-toast'));
  ...
  it('deve atualizar o NetworkBanner ao perder e ao recuperar a conexão via NetInfo', async () => {
    ...
    await act(() => {
      if (mockNetworkChangeCallback) {
        mockNetworkChangeCallback({ isConnected: true, isInternetReachable: false });
      }
    });
    ...
    await act(() => {
      if (mockNetworkChangeCallback) {
        mockNetworkChangeCallback({ isConnected: true, isInternetReachable: true });
      }
    });
