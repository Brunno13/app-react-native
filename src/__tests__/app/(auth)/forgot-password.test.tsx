...
type ForgotPasswordFormMockProps = React.ComponentProps<(typeof import('@/features/auth'))['ForgotPasswordForm']>;
    ...
    ForgotPasswordForm: ({ onResetPassword, onNavigateToLogin, loading }: ForgotPasswordFormMockProps) => (
      <View testID="mock-forgot-form">
        ...
        <TouchableOpacity
          ...
          onPress={() => { void onResetPassword('teste@email.com'); }}
        />
...
describe('ForgotPasswordRoute (Camada App)', () => {
  ...
  it('deve acionar o router.back() quando o formulário solicitar a navegação para o login', async () => {
    ...
    await act(async () => {
      await fireEvent.press(getByTestId('trigger-back'));
    });
  ...
  it('deve acionar a função forgetPassword repassando o email recebido do formulário', async () => {
    ...
    await act(async () => {
      await fireEvent.press(getByTestId('trigger-reset'));
    });
